...
export const Route = createFileRoute("/arcade_/rooms_/$id")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Waiting room for a private Fanzeno arcade match. Pick your subject, mark ready, and the host starts." },
...
function RoomLobby() {
  ...
  const { data: room } = useQuery({ queryKey: ["arcade-room", id], queryFn: () => fetchRoom(id), enabled: !!user });
  const { data: players = [] } = useQuery({ queryKey: ["arcade-room-players", id], queryFn: () => fetchRoomPlayers(id), enabled: !!user });
  ...
  const launch = (mode: string) => {
    if (mode === "sports-mastermind") void navigate({ to: "/arcade/mastermind", search: { room: id }, replace: true });
    else void navigate({ to: "/arcade/quiz-race", search: { game: mode === "quiz-ludo" ? "ludo" : "snakes", room: id }, replace: true });
  };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "arcade_room_players", filter: `room_id=eq.${id}` }, refresh)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "arcade_rooms", filter: `id=eq.${id}` }, (e) => {
        refresh();
  ...
  const copyCode = async () => {
    if (!room) return;
    try {
      await navigator.clipboard.writeText(`${window.location.origin}/arcade/rooms?code=${room.code}`);
  ...
  const myCategories = me?.sport_id ? groupCompetitions(competitions.filter((c) => c.sport_id === me.sport_id)) : [];
  ...
  return (
    <Shell title={room ? modeName(room.mode_slug) : "Private room"}>
      <div className="panel mt-6 flex items-center justify-between gap-3 p-4">
        <div>
          <p className="text-[0.58rem] font-black uppercase tracking-[0.18em] text-muted-foreground">Room code</p>
      ...
      <div className="mt-3 space-y-2">
        {Array.from({ length: max }, (_, seat) => {
          ...
          if (!p) {
            return (
              <div key={seat} className="flex items-center gap-3 rounded-2xl border border-dashed border-border p-3 opacity-70">
          ...
          return (
            <div key={seat} className={`panel flex items-center gap-3 p-3 ${ready ? "border-primary/60" : ""}`}>
              <Avatar id={p.avatar_id} />
              <div className="min-w-0 flex-1">
                <p className="flex items-center gap-2 text-sm font-bold">
                  <span className="truncate">{p.display_name}</span>
                  {p.user_id === room?.host_id && (
                    <span className="rounded-full bg-gold/15 px-1.5 py-0.5 text-[0.5rem] font-black uppercase tracking-[0.16em] text-gold">Host</span>
                  )}
                  {p.user_id === user?.id && <span className="text-[0.55rem] uppercase tracking-[0.14em] text-muted-foreground">you</span>}
                </p>
                <p className="text-xs text-muted-foreground">
                  {sportName} · {p.category_key ? myCategoryLabel(competitions, p.sport_id, p.category_key) : "All categories"}
                </p>
              </div>
              <span className={`grid size-8 place-items-center rounded-full ${ready ? "bg-primary text-primary-foreground" : "bg-surface text-muted-foreground"}`}>
            ...
            <div className="mt-1.5 flex gap-1.5 overflow-x-auto pb-1">
              <button
                ...
                className={`shrink-0 rounded-full border px-2.5 py-1 text-[0.58rem] font-bold uppercase tracking-[0.1em] ${
                  !me.category_key ? "border-gold bg-gold/15 text-foreground" : "border-border text-muted-foreground"
                }`}
              >
                All categories
              </button>
              {myCategories.map((g) => (
                <button
                  ...
                  className={`shrink-0 rounded-full border px-2.5 py-1 text-[0.58rem] font-bold uppercase tracking-[0.1em] ${
                    me.category_key === g.key ? "border-gold bg-gold/15 text-foreground" : "border-border text-muted-foreground"
                  }`}
                >
                  {g.label}
                </button>
      ...
      <p className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
        ...
        Players see the active question and the submitted answer, but only the active player&apos;s device can answer.
      </p>
...
function myCategoryLabel(
  competitions: Array<{ sport_id: string; category_key: string | null; category_label: string | null }>,
  sportId: string | null,
  key: string,
) {
  return competitions.find((c) => c.sport_id === sportId && c.category_key === key)?.category_label ?? key;
}
