...
import { BookOpenCheck, ExternalLink, Gem, ShieldCheck, Undo2 } from "lucide-react";
...
export function QuestionReviewPanel() {
  ...
  const { data, isLoading, refetch } = useQuery({ queryKey: ["admin-questions", status], queryFn: () => fetchQuestions(status) });
  ...
  const act = async (id: string, fn: "publish_question" | "unpublish_question") => {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success(fn === "publish_question" ? "Published to the fair selector" : "Returned to pending");
      void refetch();
    }
  };
  ...
  const grant = async () => {
    ...
    if (!term) return;
    const { data: profile } = await supabase.from("profiles").select("id, display_name").ilike("display_name", term).maybeSingle();
    if (!profile) {
      toast.error("No player with that display name");
      return;
    }
    const { error } = await supabase.rpc("grant_pro_lifetime", { p_user_id: profile.id, p_reason: "ops console grant" });
    if (error) toast.error(error.message);
  ...
  return (
    <>
      <div className="panel mt-5 divide-y divide-border/70">
        ...
        {(data ?? []).map((q) => {
          const accepted = ((q.answer_rule as { accepted?: string[] } | null)?.accepted ?? []) as string[];
              ...
              <div className="flex flex-wrap items-start gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-[0.6rem] font-black uppercase tracking-[0.16em] text-muted-foreground">
                    {(q.sports as { name: string } | null)?.name} · {q.category_key} · {q.format_key} · p{Math.round((q.difficulty_percentile ?? 0) * 100)}
                  </p>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    ...
                    {accepted.length > 1 && <> · also {accepted.filter((a) => a !== en(q.answer_display_i18n)).join(", ")}</>}
                  </p>
                  {en(q.clue_i18n) && <p className="text-xs text-muted-foreground">Clue: {en(q.clue_i18n)}</p>}
                  {q.source_url ? (
                    <a href={q.source_url} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-primary hover:underline">
                ...
                {status === "pending" ? (
                  <Button size="sm" disabled={!ready || busy === q.id} onClick={() => void act(q.id, "publish_question")}>
                    <ShieldCheck className="size-4" /> Publish
                  </Button>
                ) : (
                  <Button size="sm" variant="outline" disabled={busy === q.id} onClick={() => void act(q.id, "unpublish_question")}>
      ...
      <div className="panel mt-5 p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Pro is server-verified: a flag only counts when backed by a verified purchase or a recorded staff grant. Use this for
          testers and support cases until checkout ships.
        </p>
        <div className="mt-3 flex gap-2">
          <Input value={grantEmail} onChange={(e) => setGrantEmail(e.target.value)} placeholder="Player display name" aria-label="Player display name" />
